...
function QuickEditCompany({ isShowDetail = false, alreadySet = false }) {
  ...
  const imageUploadClasses = useMemo(() => {
    ...
    if (!companyForm.image) {
      return defaultStyle + " border-dashed border-2 border-black ";
    }
  ...
  return (
    <div className="bg-white rounded-xl p-4 mt-4">
      ...
      <div className="mt-3">
        <Button style={{border:"3px solid black" ,borderRadius:"10px"}} onClick={submitHandler} block={1}>
          <span className="inline-block text-black ml-2"> Submit </span>
        </Button>
      </div>
    </div>
  );
